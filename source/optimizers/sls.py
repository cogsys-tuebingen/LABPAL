...
class Sls(torch.optim.Optimizer):
    ...
    def step(self, closure):
        ...
        self.zero_grad()  # @max to be sure. it might be that the other training loop might not call this
        loss, logits = closure_deterministic()
        loss.backward()
            ...
            with torch.no_grad():
                if grad_norm >= 1e-8:
                    ...
                    for e in range(100):
                        ...
                        loss_next, _ = closure_deterministic()
                        self.state['n_forwards'] += 1
