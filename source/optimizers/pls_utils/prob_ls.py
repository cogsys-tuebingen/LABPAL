...
import numpy as np
import source.optimizers.pls_utils.gaussian_process as gaussian_process
import source.optimizers.pls_utils.utils as utils
...
class ProbLSOptimizer(object):
    ...
    def accept(self, accept_func):
        ...
        # @max
        if np.isnan(f_raw) or np.isinf(f_raw) or np.isnan(df_raw) or np.isinf(df_raw) or np.isnan(fvar_raw) or np.isinf(
                fvar_raw) or np.isnan(dfvar_raw) or np.isinf(dfvar_raw):
            f_raw = 100.0
            df_raw = 10.0
            fvar_raw = 1.0  # Todo reasonable valeus
            dfvar_raw = 1.0
            self.abort_status = 1
    ...
    def find_next_t(self):
        ...
        if self.verbose: print("\t * Best candidate is", candidates[ind_best], "(was candidate", ind_best, "/",
                               len(candidates) - 1, ")")
    ...
    def proceed(self, accept_func, adv_eval_func):
        ...
        *pass_to_func_args are arguments that are passed to the function interface,
        ...
        if self.check_for_acceptance():
            ...
            loss, step_size = self.accept(accept_func)
